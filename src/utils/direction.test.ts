...
describe("Direction Utils", () => {
  ...
  it("Returns the offset from a direction", () => {
    ...
    const east = Direction.East;
    const west = Direction.West;
    expect(DirectionUtils.offsetFromDirection(forward, north)).toEqual({ x: 0, y: 0, z: -1 });
    expect(DirectionUtils.offsetFromDirection(forward, south)).toEqual({ x: 0, y: 0, z: 1 });
    expect(DirectionUtils.offsetFromDirection(forward, east)).toEqual({ x: 1, y: 0, z: 0 });
    expect(DirectionUtils.offsetFromDirection(forward, west)).toEqual({ x: -1, y: 0, z: 0 });
  });
