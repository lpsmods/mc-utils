...
export class SearchPage extends CustomPage {
  static typeId = AddonUtils.makeId("search");
    ...
    const form: ModalForm = {
      ...
      options: {
        ...
        include_titles: {
          ...
          label: event.ctx.t(
            event,
            this.options.include_titles_label ?? "#search.include_titles",
          ),
...
export class InfoBookComponent extends ItemBaseComponent {
    ...
    return (
      JSON.parse(
        event.player.getDynamicProperty(
          event.itemStack.typeId + ".history",
        ) as string,
      ) ?? []
    );
  }
  ...
  setHistory(event: InfoBookEvent, history: string[]) {
    event.player.setDynamicProperty(
      event.itemStack.typeId + ".history",
