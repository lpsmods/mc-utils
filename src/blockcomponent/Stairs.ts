...
export class StairsComponent extends BlockBaseComponent {
  ...
  getStairsShape(block: Block, options: StairsOptions): string {
    ...
    if (
      this.isStairs(blockState) &&
      BlockUtils.matchState(block, blockState, options.half_state) &&
      WorldUtils.getAxis((direction2 = blockState.permutation.getState(options.direction_state) as string)) !=
        WorldUtils.getAxis(block.permutation.getState(options.direction_state) as string) &&
      this.isDifferentOrientation(block, WorldUtils.getOpposite(direction2), options)
    ) {
      if (direction2 == WorldUtils.rotateYCounterclockwise(direction)) {
        return "inner_right";
      }
      return "inner_left";
    }
    ...
    if (
      ...
      WorldUtils.getAxis((direction3 = blockState2.permutation.getState(options.direction_state) as string)) !=
        WorldUtils.getAxis(block.permutation.getState(options.direction_state) as string) &&
      this.isDifferentOrientation(block, direction3, options)
    ) {
      if (direction3 == WorldUtils.rotateYCounterclockwise(direction)) {
        return "outer_right";
      }
      return "outer_left";
    }
