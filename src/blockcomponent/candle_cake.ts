...
export class CandleCakeComponent extends BlockBaseComponent {
  static typeId = AddonUtils.makeId("candle_cake");
  ...
  getCakeBlock(block: Block, options: CandleCakeOptions): string {
    return (
      options.block ?? TextUtils.stripAll(block.typeId, CANDLES, undefined, "_")
    );
  }
  ...
  getCandleItem(block: Block, options: CandleCakeOptions): string {
    ...
    for (const color of COLORS) {
      ...
      if (id.path.includes(colorCandle) || id.path.includes(candleColor))
        return colorCandle;
    }
    return "candle";
  }
  ...
  setLit(
    ...
    event.block.setPermutation(
      event.block.permutation.withState(options.lit_state, value),
    );
  }
  ...
  eat(
    ...
    event.block.dimension.spawnItem(
      new ItemStack(options.candle),
      event.block.center(),
    );
    const BLOCK = BlockPermutation.resolve(
      this.getCakeBlock(event.block, options),
    );
