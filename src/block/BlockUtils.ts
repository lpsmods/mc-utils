...
export class BlockUtils {
   ...
   * @returns {Block}
   */
  static setType(block: Block, typeId?: string): void {
    block.setPermutation(
      BlockPermutation.resolve(typeId ?? "air", block.permutation.getAllStates())
    );
  }
   ...
   * @returns {Oxidization}
   */
  static guessOxidization(block: Block | string): Oxidization {
   ...
   * @returns {Vector3[]}
   */
  static getExposedBlocks(positions: Vector3[], direction: "above" | "below" | "side" = "above") {
  ...
  static matchAny(block: Block, blockNames: string[]): boolean {
    return blockNames.some((blockName) => {
